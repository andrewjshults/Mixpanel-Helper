...
var mpTracker = (function($, domElem){
    ...
    mpTracker.version = '0.3';
    mpTracker.separator = '|';
    mpTracker.errorTimeout = 300;
    ...
    mpTracker.track = function(mpEvent, mpProperties, mpCallback) {
        ...
        if(typeof mpCallback !== 'function') {
            mpCallback = function() {};
        }
        
        var now = new Date(),
        eventId = mpEvent + '-' + now.getTime();

        lastTrackedId = eventId;
        mpq.track(mpEvent, mpProperties, function() {
            mpTracker.debug('Track: ' + eventId + ' completed successfully');
            lastTrackedId = null;
            mpCallback(true);
        });
        
        window.setTimeout(function() {
            //If the lastTrackedId hasn't changed in the error timeout, assume
            //that the track event timed out and fire the error event
            if(eventId === lastTrackedId) {
                mpTracker.debug('Track: ' + eventId + ' errored');
                mpCallback(false);
            }
        }, mpTracker.errorTimeout);
